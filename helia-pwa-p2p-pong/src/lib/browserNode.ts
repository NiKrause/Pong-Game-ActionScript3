...
export type ProtocolHandler = (info: {
  stream: Stream
  peerId: string
...
export type PubsubDiscoveryUiHooks = {
   ...
   * `fromPeerId` is the libp2p peer that signed the message (compare to the configured relay peer id to spot the relay).
   */
...
export class ConnectivityBrowserNode {
  ...
  private hookGossipsubInboundDiscoveryUi(libp2p: Libp2p, discoveryTopic: string): void {
    ...
    const handleMsg = (msg: PubsubMessage): void => {
      ...
      this.pushDebugEvent({
        ...
        peerId: msg.from.toString(),
      })
      try {
        cb({ fromPeerId: msg.from.toString() })
      } catch {
        // ignore
      }
    }
  ...
  private hookCircuitRelayReservationUi(libp2p: Libp2p): void {
    if (this.reservationStoreHooked) return
  ...
  private async upsertDiscoveryRow(libp2p: Libp2p, peerIdStr: string, multiaddrs: string[]): Promise<void> {
    ...
    if (existing != null) {
      ...
      existing.autoDialEligible = true
      existing.detail = multiaddrs.length > 0 ? undefined : 'dialing by peer id'
    ...
    const row: DiscoveryRow = {
      ...
      autoDialEligible: true,
      autoDial: 'pending',
      detail: multiaddrs.length > 0 ? undefined : 'dialing by peer id',
    }
  ...
  async dialMultiaddrForDebug(maStr: string): Promise<void> {
    const node = this.libp2p
    if (!node) throw new Error('node not started')
    this.pushDebugEvent({
      type: 'debug:dial:start',
      at: Date.now(),
      addr: maStr,
    })
    await node.dial(multiaddr(maStr))
  ...
  async handleProtocol(protocol: string, handler: ProtocolHandler): Promise<void> {
    const node = this.libp2p
    if (!node) throw new Error('node not started')

    await node.handle(
      protocol,
      async ({ stream, connection }: IncomingStreamData) => {
        await handler({
          stream,
          peerId: connection.remotePeer.toString(),
  ...
  async dialPeerProtocol(
    peerIdStr: string,
    protocol: string,
    preferredAddrs: string[] = []
  ): Promise<{ stream: Stream; remoteAddr: string | null }> {
    const node = this.libp2p
    if (!node) throw new Error('node not started')

    let lastError: unknown = null
    const ordered = [...preferredAddrs].sort((a, b) => scoreDialMultiaddr(b) - scoreDialMultiaddr(a))
    for (const addr of ordered) {
      try {
        const ma = multiaddr(addr)
        this.pushDebugEvent({
          type: 'protocol:dial:start',
          at: Date.now(),
          peerId: peerIdStr,
          addr: ma.toString(),
          detail: protocol,
        ...
        this.pushDebugEvent({
          type: 'protocol:dial:ok',
          at: Date.now(),
          peerId: peerIdStr,
          addr: ma.toString(),
          detail: protocol,
        })
        return { stream, remoteAddr: ma.toString() }
      } catch (error) {
        this.pushDebugEvent({
          type: 'protocol:dial:error',
          at: Date.now(),
          peerId: peerIdStr,
          addr: addr,
    ...
    try {
      ...
      return {
        stream,
        remoteAddr: this.getPreferredPeerConnectionAddr(peerIdStr),
      }
    } catch (error) {
      this.pushDebugEvent({
        type: 'protocol:dial:error',
  ...
  getTopicSubscribers(): string[] {
    const node = this.libp2p
    if (node == null) return []
    const svc = node.services?.pubsub as { getSubscribers?: (topic: string) => Array<{ toString: () => string }> } | undefined
    if (svc == null || typeof svc.getSubscribers !== 'function') return []
    try {
      return svc.getSubscribers(this.topic).map((peerId) => peerId.toString())
    } catch {
      return []
    }
  }
  ...
  getDebugSnapshot(): BrowserNodeDebugSnapshot {
    const node = this.libp2p
    const peerIds = node?.getPeers().map((peerId) => peerId.toString()) ?? []
    return {
      topic: this.topic,
      localPeerId: node?.peerId.toString() ?? null,
      ownMultiaddrs: this.getOwnMultiaddrs(),
      peerCount: peerIds.length,
      discoveryRows: [...this.discoveryMap.values()].map((row) => ({
        ...row,
        discoveryAddrs: [...row.discoveryAddrs],
      })),
      connections: peerIds.flatMap((peerId) =>
        this.getPeerConnectionAddrs(peerId).map((addr) => ({
          peerId,
          addr,
        }))
      ),
  ...
  async publishDebugPubsubMessage(topic: string, message: Uint8Array): Promise<void> {
    const node = this.libp2p
    if (node == null) throw new Error('node not started')
    const svc = node.services?.pubsub as { publish?: (topic: string, message: Uint8Array) => Promise<unknown> } | undefined
    if (svc == null || typeof svc.publish !== 'function') {
      throw new Error('pubsub not available')
    }
    this.pushDebugEvent({
      type: 'pubsub:debug:publish:start',
      at: Date.now(),
      topic,
      detail: `bytes=${message.byteLength}`,
    })
    await svc.publish(topic, message)
    this.pushDebugEvent({
      type: 'pubsub:debug:publish:ok',
      at: Date.now(),
      topic,
      detail: `bytes=${message.byteLength}`,
    })
